"""DP2DP3 deploy_film_online_3model_policy.py

在线 3 模型 DA3-FiLM 部署脚本 (DINOv3 + CroCo + DA3)
=====================================================

架构说明:
  - 加载 DINOv3 + CroCo + DA3 三个 backbone (vs 离线版 4 个)
  - Encoder: DA3Film3ModelEncoder (concat_proj + FiLM 调制)
  - Policy:  Diffusion Policy (与离线 4 模型版完全一致的 DP head)
  - 推理: RGB -> DINOv3/CroCo/DA3 tokens -> concat_proj -> FiLM -> DP -> action

RoBoTwin 标准接口:
  get_model(usr_args) -> Film3ModelOnline
...
# 3 模型在线特征提取器
from features_common.depth_guided_film_online_3model.extractors_3model import ThreeModelExtractors
from features_common.depth_guided_film_online_3model.encoder_film_3model import DA3Film3ModelEncoder
...
# ============================================================
# Policy (与训练脚本中 DA3Film3ModelPolicy 完全镜像)
# ============================================================

class Film3ModelDeployPolicy(nn.Module):
    """3 模型 DA3-FiLM + proprio + Diffusion Policy (部署版)."""

    def __init__(
        self,
        fusion_encoder: DA3Film3ModelEncoder,
        proprio_dim: int = 14,
        action_dim: int = 14,
        horizon: int = 8,
        n_obs_steps: int = 3,
...
# ============================================================
# Film3ModelOnline: 完整的模型包装器
# ============================================================

class Film3ModelOnline:
    """3 模型 FiLM 在线部署包装器, 兼容 RoBoTwin eval 接口."""
    ...
    def __init__(self, usr_args):
        ...
        # Checkpoint 路径搜索
        env_ckpt_dir = os.environ.get("DP2DP3_FILM_ONLINE_3MODEL_CKPT_DIR", "")
        if env_ckpt_dir and Path(env_ckpt_dir).is_dir():
            ckpt_dir = Path(env_ckpt_dir)
            print(f"[Film3Model] Using env ckpt dir: {ckpt_dir}")
        else:
            ckpt_dir_name = (
                f"{self.task_name}-{self.ckpt_setting}-{self.expert_data_num}-{self.seed}"
            )
            ckpt_roots = [
                Path(policy_dir) / "checkpoints_film_online_3model",
                Path(features_model_dir) / "checkpoints_film_online_3model",
            ]
            ckpt_dir = None
            ...
            if ckpt_dir is None:
                raise FileNotFoundError(
                    f"[Film3Model] Checkpoint dir not found. Searched:\n"
        ...
        if str(self.checkpoint_num).lower() == "best":
            ...
            if best.exists():
                ...
                if not files:
                    raise FileNotFoundError(f"[Film3Model] No .ckpt in {ckpt_dir}")
                self.ckpt_path = max(files, key=lambda f: int(f.stem))
        else:
            self.ckpt_path = ckpt_dir / f"{int(self.checkpoint_num)}.ckpt"
            if not self.ckpt_path.exists():
                raise FileNotFoundError(
                    f"[Film3Model] Checkpoint not found: {self.ckpt_path}"
                )

        print(f"[Film3Model] Loading checkpoint: {self.ckpt_path}")
        self._load_models()

        self.obs_buffer = deque(maxlen=self.n_obs_steps)
        self.action_queue = deque()
        print(
            f"[Film3Model] Ready: horizon={self.horizon}, "
    ...
    def _load_models(self):
        ...
        print(f"[Film3Model] Encoder config: {enc_cfg}")

        # 1. 加载 DINOv3 + CroCo + DA3
        print("[Film3Model] Loading DINOv3 + CroCo + DA3...")
        self.feature_extractors = ThreeModelExtractors(gpu_id=self.gpu_id)

        # 2. 创建 Encoder
        print("[Film3Model] Creating DA3Film3ModelEncoder...")
        fusion_encoder = DA3Film3ModelEncoder(
            semantic_in_dims=tuple(enc_cfg.get("semantic_in_dims", [768, 1024])),
        ...
        # 3. 创建 Policy
        print("[Film3Model] Creating Policy...")
        n_action_steps = config.get("data", {}).get("n_action_steps", 6)
        self.policy = Film3ModelDeployPolicy(
        ...
        # 4. 加载权重
        print("[Film3Model] Loading weights...")
        ...
        print("[Film3Model] All models loaded successfully!")
...
def get_model(usr_args):
    """RoBoTwin 标准接口: 获取模型."""
    return Film3ModelOnline(usr_args)
