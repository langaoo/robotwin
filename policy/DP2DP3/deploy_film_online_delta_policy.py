"""DP2DP3 deploy_film_online_delta_policy.py

FiLM 时序差分部署脚本 (DINOv3 + DA3 时序差分调制)
===================================================

架构说明:
  - DINOv3 + DA3 两个 backbone
  - Encoder: DA3Film2ModelEncoderDelta (时序差分 geo_vec)
  - Policy:  Diffusion Policy (与 FiLM v1 完全一致)
  - 推理时 To 帧将在 encoder.forward 内部自动计算差分

RoBoTwin 标准接口:
  get_model(usr_args) -> FilmDeltaOnlineModel
...
class FilmDeltaDeployPolicy(nn.Module):
    def __init__(
        self,
...
# ============================================================
# FilmDeltaOnlineModel
# ============================================================

class FilmDeltaOnlineModel:
    """FiLM 时序差分在线部署包装器, 兼容 RoBoTwin eval 接口."""
    ...
    def __init__(self, usr_args):
        ...
        env_ckpt_dir = os.environ.get("DP2DP3_FILM_DELTA_CKPT_DIR", "")
        if env_ckpt_dir and Path(env_ckpt_dir).is_dir():
            ...
            ckpt_roots = [
                Path(policy_dir) / "checkpoints_film_online_delta",
                Path(features_model_dir) / "checkpoints_film_online_delta",
            ]
            ckpt_dir = None
            ...
            if ckpt_dir is None:
                raise FileNotFoundError(
                    f"[FilmDelta] Checkpoint dir not found. Searched:\n"
        ...
        if str(self.checkpoint_num).lower() == "best":
            ...
            if best.exists():
                ...
                if not files:
                    raise FileNotFoundError(f"[FilmDelta] No .ckpt in {ckpt_dir}")
                self.ckpt_path = max(files, key=lambda f: int(f.stem))
        else:
            self.ckpt_path = ckpt_dir / f"{int(self.checkpoint_num)}.ckpt"
            if not self.ckpt_path.exists():
                raise FileNotFoundError(f"[FilmDelta] Checkpoint: {self.ckpt_path}")

        print(f"[FilmDelta] Loading checkpoint: {self.ckpt_path}")
        self._load_models()

        self.obs_buffer = deque(maxlen=self.n_obs_steps)
        self.action_queue = deque()
        print(f"[FilmDelta] Ready: horizon={self.horizon}, n_obs_steps={self.n_obs_steps}")
    ...
    def _load_models(self):
        ...
        print(f"[FilmDelta] Encoder config: {enc_cfg}")

        print("[FilmDelta] Loading DINOv3 + DA3...")
        self.feature_extractors = TwoModelExtractors(gpu_id=self.gpu_id)

        print("[FilmDelta] Creating DA3Film2ModelEncoderDelta...")
        fusion_encoder = DA3Film2ModelEncoderDelta(
        ...
        self.policy = FilmDeltaDeployPolicy(
        ...
        print("[FilmDelta] All models loaded!")
...
def get_model(usr_args):
    return FilmDeltaOnlineModel(usr_args)
